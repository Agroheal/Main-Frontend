...
const FarmAdmin = () => {
  ...
  const calcSlotFee = (r: Pick<FarmRecord, "farm_slots">) => {
    const slotFeeRate =
      selectedFarm?.project_category === "Mushroom Village" ? 1000 : 2000;
  ...
  const fetchRecords = async (farmId: string) => {
    ...
    if (recRes.error) {
      showToast({
        ...
        description: recRes.error.message,
      });
    } else {
      const farm = farms.find((f) => f.id === farmId);
      const enriched = await enrichRecordsWithBatches(
        recRes.data || [],
        farm?.project_category || "Gingertown",
      );
      setRecords(enriched);
    }
  ...
  const lookupUserByEmail = async (email: string) => {
    ...
    if (userId) {
      const { data: payments, error: payErr } = await supabase.rpc(
  ...
  const handleDeleteFarm = async (farmId: string) => {
    ...
    showToast({
      variant: "success",
      title: "Farm group and all records deleted",
    });
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
              <div>
                ...
                <p className="text-gray-600">
                  Manage all farm groups and member records
                </p>
              </div>
              <Button
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b bg-gray-50">
                                ...
                                <th className="text-center p-2 no-print">
                                  Actions
                                </th>
                              </tr>
                            </thead>
